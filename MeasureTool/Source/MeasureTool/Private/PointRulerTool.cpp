...
#define LOCTEXT_NAMESPACE "PointRulerTool"
...
bool FPointRulerTool::HandleClick(FEditorViewportClient* InViewportClient, HHitProxy* HitProxy, const FViewportClick& Click)
{
	if (HitSuccess)
	{
		...
		else
		{
			...
			// ¾àÀë´óÓÚ0.01cmÏÔÊ¾
			if (!(Measures[Measures.Num() - 1]->StartLocation - BestLocation).IsNearlyZero(0.01f))
			{
				AMeasureActor::GenerateMeasureActor(Measures[Measures.Num() - 1]->StartLocation, BestLocation);
			}
			bIsMeasuring = false;
		}
		return true;
